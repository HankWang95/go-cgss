...
func (server *CenterServer) Handle(method, params string) *ipc.Responce {
	switch method {
	case "addplayer":
		err := server.addPlayer(params)
		if err != nil {
			return &ipc.Responce{Code: err.Error()}
		}
	case "removeplayer":
	...
	case "listplayer":
		...
		return &ipc.Responce{"200", players}
	case "broadcast":
		err := server.broadcast(params)
		if err != nil {
			return &ipc.Responce{Code: err.Error()}
		}
		return &ipc.Responce{Code: "200"}
	default:
		return &ipc.Responce{Code: "404", Body: method + ":" + params}
	}
